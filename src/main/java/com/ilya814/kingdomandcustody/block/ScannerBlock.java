    ...
    protected InteractionResult useWithoutItem(BlockState state, Level level, BlockPos pos, Player player, BlockHitResult hit) {
        ...
        if (held.is(Items.STICK)) {
            if (player.isShiftKeyDown()) {
                ...
                player.sendSystemMessage(net.minecraft.network.chat.Component.literal("§6[Scanner] §fCorner 2: " + player.blockPosition().toShortString()));
            } else {
                scanner.setPos1(player.blockPosition());
                player.sendSystemMessage(net.minecraft.network.chat.Component.literal("§6[Scanner] §fCorner 1: " + player.blockPosition().toShortString()));
            }
            scanner.setChanged();
        ...
        if (held.is(Items.LEVER)) {
            ...
            player.sendSystemMessage(net.minecraft.network.chat.Component.literal("§6[Scanner] §fNow right-click any placed lever to pair it!"));
            return InteractionResult.SUCCESS;
        }
